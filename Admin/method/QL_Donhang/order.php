...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action']) && $_POST['action'] == 'toggle_payment_status') {
        ...
        if ($id_order) {
            ...
            $db->bind(':id', $id_order);
            $db->execute();
            $msg = ($new_status == 1) ? 'Đã Thanh Toán' : 'Chưa Thanh Toán';
        ...
        <table class="um-table" style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="background-color: #f8f9fa; border-bottom: 2px solid #dee2e6;">
                    ...
                    <th style="padding: 12px; text-align: center;">Trạng Thái TT</th>
                    ...
                    <td style="padding: 12px; vertical-align: middle; text-align: center;">
                        <div style="display: flex; justify-content: center; gap: 8px;">
                            ...
                            <form method="POST" style="margin: 0;">
                                ...
                                <?php if ($o['trang_thai_thanh_toan'] == 1): ?>
                                    <button type="submit" style="background-color: #6c757d; color: white; padding: 6px 12px; border-radius: 4px; border: none; cursor: pointer; font-size: 0.9em; transition: 0.2s;" onmouseover="this.style.opacity='0.8';" onmouseout="this.style.opacity='1';" onclick="return confirm('Chuyển trạng thái đơn hàng này về Chưa thanh toán?');">
                                        <i class="fas fa-times"></i> Hủy duyệt
                                    </button>
                                <?php else: ?>
                                    <button type="submit" style="background-color: #28a745; color: white; padding: 6px 12px; border-radius: 4px; border: none; cursor: pointer; font-size: 0.9em; transition: 0.2s;" onmouseover="this.style.opacity='0.8';" onmouseout="this.style.opacity='1';" onclick="return confirm('Xác nhận Cập nhật thành Đã thanh toán?');">
                                        <i class="fas fa-check"></i> Duyệt TT
                                    </button>
                                <?php endif; ?>
                            </form>
                        </div>
